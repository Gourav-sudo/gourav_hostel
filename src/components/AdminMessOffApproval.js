...
const AdminMessOffApproval = () => {
  const [applications, setApplications] = useState([]);

  const fetchApplications = async () => {
    const res = await axios.get("http://localhost:5000/api/mess-off/requests?status=pending");
    setApplications(res.data);
  };

  const handleApproval = async (id, status) => {
    try {
      await axios.put(`http://localhost:5000/api/mess-off/update/${id}`, { status: status });
  ...
  return (
    <div>
      <h2>Approve/Reject Mess Off Applications</h2>
      <ul>
        {applications.map((app) => (
          <li key={app._id}>
            {app.studentId} applied for {app.startDate} to {app.endDate}
            <button onClick={() => handleApproval(app._id, "Approved")}>
              Approve
            </button>
            <button onClick={() => handleApproval(app._id, "Rejected")}>
              Reject
            </button>
          </li>
        ))}
...
export default AdminMessOffApproval;
